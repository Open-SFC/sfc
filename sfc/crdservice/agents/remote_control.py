...
from nscs.crdclient.v2_0 import client as qclient
    ...
    def process_config_request(self,fileno):
        ...
        try:
		    ...
		    LOG.debug('sending data to VM............\n\n%s\n\n' % self.port_fds[fileno]['conf_up_req'])
                ...
                try:
                    qc = crdclient(self.user,self.password,self.tenant,self.auth_url,self.endpoint_url)
                    LOG.debug("crd client created ......%s\n\n" % str(qc))
                    q_data_recv=str(getattr(qc,req['method'])(**req['kwargs']))
                    LOG.debug('data received from nscs.crdservice client = %s\n\n' % q_data_recv)
                    self.port_fds[fileno]['fd'].send(q_data_recv)
                    return
                except Exception,msg:
                    LOG.error(_('Crdclient Exception...\n\t%s'),msg)
...
def crdclient(username,password,tenant,auth_url,endpoint_url):
