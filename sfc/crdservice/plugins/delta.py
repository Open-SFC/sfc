...
from sfc.crdservice.db import delta
...
class SfcDelta(object):
    ...
    def get_networkfunctions_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_networkfunctions_deltas(context, filters=filters, fields=fields)
    
    def get_categories_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_categories_deltas(context, filters=filters, fields=fields)
    
    def get_vendors_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_vendors_deltas(context, filters=filters, fields=fields)
    
    def get_appliances_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_appliances_deltas(context, filters=filters, fields=fields)
    
    def get_config_handles_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_config_handles_deltas(context, filters=filters, fields=fields)
    
    def get_category_networkfunctions_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_category_networkfunctions_deltas(context, filters=filters, fields=fields)
    
    def get_chains_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_chains_deltas(context, filters=filters, fields=fields)
    
    def get_chain_appliances_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_chain_appliances_deltas(context, filters=filters, fields=fields)
    
    def get_chain_bypass_rules_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_chain_bypass_rules_deltas(context, filters=filters, fields=fields)
    
    def get_chainsets_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_chainsets_deltas(context, filters=filters, fields=fields)
    
    def get_chainrules_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_chainrules_deltas(context, filters=filters, fields=fields)
    
    def get_chainmaps_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_chainmaps_deltas(context, filters=filters, fields=fields)
    
    def get_appliance_instances_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_appliance_instances_deltas(context, filters=filters, fields=fields)
    
    def create_chainset_zone_deltas(self, context, data_dict):
        result_delta = self.deltadb.create_chainset_zone_deltas(context, data_dict)
        return result_delta
    
    def get_chainset_zone_deltas(self, context, filters=None, fields=None):
        return self.deltadb.get_chainset_zone_deltas(context, filters=filters, fields=fields)
    ####Delta############
    ...
    def sfc_init(self, ctx, version, hostname):
	    ...
	    #Chainset Zone delta
            zones_diff = self.db.get_chainset_zones(ctx, filters=None,
                                                          fields=None)
            for zone in zones_diff:
                fil = {}
                id = zone['id']
                fil['zone_id'] = [id]
                fil['operation'] = ['create']
                CompDelta = self.deltadb.get_chainset_zones_deltas(ctx, fil)
                arr = sorted(CompDelta, key=lambda t: t['version_id'],
                             reverse=True)
                verid = arr[0]['version_id']
                zone.update(
                    {'zone_id': zone['id'], 'operation': 'create',
                     'version_id': current_version})
                zone_message = {}
                zone_message.update(
                    {'method': 'create_chainset_zone', 'payload': zone})
                delta[verid] = zone_message
            ...
            for nwchainmap in nwchainmap_diff:
                ...
                arr = sorted(CompDelta, key=lambda t: t['version_id'],
                             reverse=True)
		LOG.debug(_("ARRAYTESTETTETETETET = %s"), arr)
                verid = arr[0]['version_id']
                nwchainmap.update(
