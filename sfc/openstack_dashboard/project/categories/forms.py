...
class BaseCategoryfunctionForm(forms.SelfHandlingForm):
    ...
    def __init__(self, request, *args, **kwargs):
        super(BaseCategoryfunctionForm, self).__init__(
            request, *args, **kwargs)
        # Populate Categoryfunctions choices
        networkfunction_choices = []
        tenant_id = self.request.user.tenant_id
        for networkfunction in api.sfc.networkfunction_list_for_tenant(request, tenant_id):
            networkfunction_choices.append(
                (networkfunction.id, networkfunction.name))
...
class CreateCategory(BaseCategoryfunctionForm):
    ...
    def handle(self, request, data):
        try:
            ...
            sg = api.sfc.category_create(request,
                                         name=data['name'],
                                         description=data['description'],
                                         shared=0,
                                         category_networkfunctions=networkfunctions)

            # for networkfunction in networkfunctions:
            #    cn = api.sfc.category_networkfunction_create(request,
            #                               category_id=sg.id,
...
class UpdateCategory(forms.SelfHandlingForm):
    ...
    failure_url = 'horizon:project:categories:index'

    def handle(self, request, data):
        try:
            category = api.sfc.category_modify(request, data['category_id'],
