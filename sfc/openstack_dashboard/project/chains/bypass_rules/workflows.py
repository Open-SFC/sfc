...
class CreateGeneralAction(workflows.Action):
    ...
    chain_id = forms.CharField(label=_("Chainset Id"),
...
class NwservicesAction(workflows.Action):
    nwservice_names = forms.CharField(max_length=50, label=_("Appliances"),
                                      required=True,
                                      initial="")
    nwservice_count = forms.CharField(max_length=50, label=_("Applicnces Count"),
                                      required=True,
                                      initial="")

    class Meta:
        name = ("Applicances")
        help_text = _("Add Chain appliances Details.\n"
                      ""
                      "")
...
class CreateGeneral(workflows.Step):
    ...
    contributes = ("name", "chain_id", "eth_type", "eth_value", "ip_protocol",)
...
class Nwservices(workflows.Step):
    action_class = NwservicesAction
    contributes = ("nwservice_count", "nwservice_names",)


class Addl2Rule(workflows.Workflow):
    slug = "create_rule"
    name = _("Create Bypass Rule")
    finalize_button_name = _("Create")
    success_message = _('Created Bypass Rule "%s".')
    failure_message = _('Unable to create Bypass Rule "%s".')
    ...
    def format_status_message(self, message):
        name = self.context.get('name') or self.context.get('chain_id', '')
        return message % name
    ...
    def handle(self, request, data):
        ...
        try:
            ...
            sg = api.sfc.bypass_rule_create(request, data['chain_id'],
                                              name=data['name'],
                                              src_mac_type=data[
                                                  'src_mac_type'],
                                              dest_mac_type=data[
                                              ...
                                              nwservice_count=data[
                                                  'nwservice_count'],
                                              nwservice_names=data[
                                                  'nwservice_names'],
                                              )
