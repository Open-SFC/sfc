...
class CreateNetworkfunction(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            sg = api.sfc.networkfunction_create(request,
                                                name=data['name'],
                                                description=data[
                                                    'description'],
                                                shared=0)
...
class UpdateNetworkfunction(forms.SelfHandlingForm):
    ...
    shared = forms.CharField(widget=forms.HiddenInput)
    failure_url = 'horizon:project:networkfunctions:index'

    def handle(self, request, data):
        try:
            networkfunction = api.sfc.networkfunction_modify(request, data['networkfunction_id'],
                                                             name=data['name'], description=data['description'], shared=data['shared'])
            msg = _('Networkfunction %s was successfully updated.') % data[
                'name']
            LOG.debug(msg)
