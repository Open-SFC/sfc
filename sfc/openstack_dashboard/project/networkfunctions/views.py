...
"""
Views for managing Crd Networkfunctions.
"""
import logging
...
class IndexView(tables.DataTableView):
    ...
    template_name = 'project/networkfunctions/index.html'

    def get_data(self):
        try:
            tenant_id = self.request.user.tenant_id
            networkfunctions = api.sfc.networkfunction_list_for_tenant(self.request,
                                                                       tenant_id)
        except:
            networkfunctions = []
...
class CreateNetworkfunctionView(forms.ModalFormView):
    ...
    template_name = 'project/networkfunctions/create.html'
    success_url = reverse_lazy('horizon:project:networkfunctions:index')


class UpdateNetworkfunctionView(forms.ModalFormView):
    form_class = UpdateNetworkfunction
    template_name = 'project/networkfunctions/update.html'
    context_object_name = 'networkfunction'
    success_url = reverse_lazy("horizon:project:networkfunctions:index")

    def get_context_data(self, **kwargs):
        context = super(
            UpdateNetworkfunctionView, self).get_context_data(**kwargs)
    ...
    def _get_object(self, *args, **kwargs):
        if not hasattr(self, "_object"):
            ...
            try:
                self._object = api.sfc.networkfunction_get(self.request,
                                                           networkfunction_id)
            except:
