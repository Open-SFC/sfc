...
from nscs.crdserver.manager import CrdManager as q_man
from nscs.crdserver.common import exceptions as q_exc

from sfc.crdserver.db import sfc_db
from sfc.crdserver.db import delta as sfc_delta_db
...
crd_nwservices_opts = [
    ...
    cfg.StrOpt('auth_url'),
    cfg.StrOpt('vlan_quota', default="1-100"),
]

cfg.CONF.register_opts(crd_nwservices_opts, "NWSDRIVER")
    ...
    def launch_chain(self, context, chainset_id, chainmap_id, rule_id):
        ...
        for appliance_map in appliance_maps:
            ...
            if instance_uuid:
                ...
                vlanquotas = cfg.CONF.NWSDRIVER.vlan_quota
