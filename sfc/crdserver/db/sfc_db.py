...
from nscs.crdserver.db import model_base, db_base_plugin_v2
...
class SFCConfigHandle(model_base.BASEV2, model_base.HasId,
                      model_base.HasTenant):
    """Represents a v2 crd FSL service."""
    __tablename__ = 'sfc_config_handles'
...
class SFCAppliance(model_base.BASEV2, model_base.HasId, model_base.HasTenant):
    ...
    config_handle_id = sa.Column(sa.String(36),
                                 sa.ForeignKey('sfc_config_handles.id'),
                                 nullable=False)
    config_handles = relationship(SFCConfigHandle)
...
class SFCPluginDb(db_base_plugin_v2.CrdDbPluginV2):
    ...
    def get_config_handles(self, context, filters=None, fields=None):
        return self._get_collection(context, SFCConfigHandle,
    ...
    def create_config_handle(self, context, config_handle):
        ...
        tenant_id = self._get_tenant_id_for_create(context, n)
        with context.session.begin(subtransactions=True):
            config_handle = SFCConfigHandle(tenant_id=tenant_id,
                                            id=self.__get_id(n),
                                            name=n['name'],
                                            networkfunction_id=n[
                                                'networkfunction_id'],
                                            status=n['status'],
                                            slug=n['slug'],
    ...
    def _get_config_handle(self, context, id):
        try:
            config_handle = self._get_by_id(context, SFCConfigHandle, id)
        except exc.NoResultFound:
            # NOTE(jkoelker) The PortNotFound exceptions requires net_id
            #                kwarg in order to set the message correctly
            raise q_exc.ConfigHandleNotFound(config_handle_id=id)
        except exc.MultipleResultsFound:
