...
from nscs.crdserver.db import api as qdbapi
...
from nscs.crdserver.plugins.common import constants
...
from nscs.crdserver.openstack.common.rpc.proxy import RpcProxy as rpc_proxy
...
class ns_categories_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
...
class ns_category_networkfunctions_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
...
######Vendors Delta    
class ns_vendors_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
...
class ns_appliances_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
...
class ns_metadatas_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
    metadata_id = sa.Column(sa.String(36), nullable=False)
    name = sa.Column(sa.String(50))
...
class ns_personalities_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
    personality_id = sa.Column(sa.String(36), nullable=False)
    name = sa.Column(sa.String(50))
    file_path = sa.Column(sa.String(255))
    file_content = sa.Column(sa.String(255))
    appliance_id = sa.Column(sa.String(36))
    operation = sa.Column(sa.String(255), nullable=False)
    user_id = sa.Column(sa.String(36),nullable=False)
    logged_at = sa.Column(sa.DateTime, default=datetime.datetime.now, nullable=False)
...
class ns_chains_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
...
class ns_chain_appliances_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
...
class ns_chain_configuration_associates_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
    chain_appliance_map_config_id = sa.Column(sa.String(36), nullable=False)
    name = sa.Column(sa.String(50))
    networkfunction_id = sa.Column(sa.String(36))
    chain_map_id = sa.Column(sa.String(36))
    config_handle_id = sa.Column(sa.String(36))
    operation = sa.Column(sa.String(255), nullable=False)
    user_id = sa.Column(sa.String(36),nullable=False)
    logged_at = sa.Column(sa.DateTime, default=datetime.datetime.now, nullable=False)
...
class ns_chain_bypass_rules_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
...
### Chainset for delta
class ns_chainsets_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
...
class ns_chainrules_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
...
class ns_chainmaps_delta(model_base.BASEV2, HasId, HasTenant):
    """Represents a v2 crd FSL service."""
...
class ns_appliance_instances_delta(model_base.BASEV2, HasId, HasTenant):
...
class SfcDeltaDb(db_base_plugin_v2.CrdDbPluginV2):
    ...
    def get_categories_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_categories_delta,
    ...
    def create_categories_delta(self, context, categories_delta):
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            categories_delta = ns_categories_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
                                         category_id=n['id'],
                                         name=n['name'],
                                         description=n['description'],
    ...
    def _get_categories_delta(self, context, id):
        try:
            categories_delta = self._get_by_id(context, ns_categories_delta, id)
        except exc.NoResultFound:
    ...
    def get_category_networkfunctions_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_category_networkfunctions_delta,
    ...
    def create_category_networkfunctions_delta(self, context, category_networkfunctions_delta):
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            category_networkfunctions_delta = ns_category_networkfunctions_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
    ...
    def _get_category_networkfunctions_delta(self, context, id):
        try:
            category_networkfunctions_delta = self._get_by_id(context, ns_category_networkfunctions_delta, id)
        except exc.NoResultFound:
    ...
    def get_vendors_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_vendors_delta,
    ...
    def create_vendors_delta(self, context, vendors_delta):
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            vendors_delta = ns_vendors_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
                                         vendor_id=n['id'],
                                         name=n['name'],
                                         description=n['description'],
    ...
    def _get_vendors_delta(self, context, id):
        try:
            vendors_delta = self._get_by_id(context, ns_vendors_delta, id)
        except exc.NoResultFound:
    ...
    def get_appliances_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_appliances_delta,
    ...
    def create_appliances_delta(self, context, appliances_delta):
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            appliances_delta = ns_appliances_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
                                         appliance_id=n['id'],
                                         name=n['name'],
                                         category_id=n['category_id'],
    ...
    def _get_appliances_delta(self, context, id):
        try:
            appliances_delta = self._get_by_id(context, ns_appliances_delta, id)
        except exc.NoResultFound:
    ...
    ######################### Metadata Delta ###############################   
    def _make_metadatas_delta_dict(self, metadatas_delta, fields=None):
        res = {'id': metadatas_delta['id'],
               'metadata_id': metadatas_delta['metadata_id'],
               'name': metadatas_delta['name'],
               'value': metadatas_delta['value'],
               'appliance_id': metadatas_delta['appliance_id'],
               'tenant_id': metadatas_delta['tenant_id'],
               'operation': metadatas_delta['operation'],
               'user_id': metadatas_delta['user_id'],
               'logged_at': metadatas_delta['logged_at'],
               'version_id': metadatas_delta['version_id']}
        return self._fields(res, fields)
    
    def get_metadatas_delta(self, context, id, fields=None):
        metadatas_delta = self._get_metadatas_delta(context, id)
        return self._make_metadatas_delta_dict(metadatas_delta, fields)
        
    def get_metadatas_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_metadatas_delta,
                                    self._make_metadatas_delta_dict,
                                    filters=filters, fields=fields)
    
    def create_metadatas_delta(self, context, metadatas_delta):
        n = metadatas_delta['metadatas_delta']
        LOG.error('Delta %s' % str(n))
        tenant_id = self._get_tenant_id_for_create(context, n)
        user_id = context.user_id
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            metadatas_delta = ns_metadatas_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
                                         metadata_id=n['id'],
                                         name=n['name'],
                                         value=n['value'],
                                         ...
                                         logged_at=datetime.datetime.now(),
                                         version_id=version_id)
            context.session.add(metadatas_delta)
        payload = self._make_metadatas_delta_dict(metadatas_delta)
        method = n['operation']+"_metadata"
        #rpc.fanout_cast(context,'crd-consumer', rpc_proxy.make_msg(method,payload=payload))
        return self._make_metadatas_delta_dict(metadatas_delta)
    
    def _get_metadatas_delta(self, context, id):
        try:
            metadatas_delta = self._get_by_id(context, ns_metadatas_delta, id)
        except exc.NoResultFound:
            raise q_exc.metadatas_deltaNotFound(metadatas_delta_id=id)
        except exc.MultipleResultsFound:
            LOG.error('Multiple metadatas_deltas match for %s' % id)
            raise q_exc.metadatas_deltaNotFound(metadatas_delta_id=id)
        return metadatas_delta
    
    ######################### Personality Delta ###############################   
    def _make_personalities_delta_dict(self, personalities_delta, fields=None):
        res = {'id': personalities_delta['id'],
               'personality_id': personalities_delta['personality_id'],
               'name': personalities_delta['name'],
               'file_path': personalities_delta['file_path'],
               'file_content': personalities_delta['file_content'],
               'appliance_id': personalities_delta['appliance_id'],
               'tenant_id': personalities_delta['tenant_id'],
               'operation': personalities_delta['operation'],
               'user_id': personalities_delta['user_id'],
               'logged_at': personalities_delta['logged_at'],
               'version_id': personalities_delta['version_id']}
        return self._fields(res, fields)
    
    def get_personalities_delta(self, context, id, fields=None):
        personalities_delta = self._get_personalities_delta(context, id)
        return self._make_personalities_delta_dict(personalities_delta, fields)
        
    def get_personalities_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_personalities_delta,
                                    self._make_personalities_delta_dict,
                                    filters=filters, fields=fields)
    
    def create_personalities_delta(self, context, personalities_delta):
        n = personalities_delta['personalities_delta']
        LOG.error('Delta %s' % str(n))
        tenant_id = self._get_tenant_id_for_create(context, n)
        user_id = context.user_id
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            personalities_delta = ns_personalities_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
                                         personality_id=n['id'],
                                         name=n['name'],
                                         ...
                                         logged_at=datetime.datetime.now(),
                                         version_id=version_id)
            context.session.add(personalities_delta)
        payload = self._make_personalities_delta_dict(personalities_delta)
        method = n['operation']+"_personality"
        #rpc.fanout_cast(context,'crd-consumer', rpc_proxy.make_msg(method,payload=payload))
        return self._make_personalities_delta_dict(personalities_delta)
    
    def _get_personalities_delta(self, context, id):
        try:
            personalities_delta = self._get_by_id(context, ns_personalities_delta, id)
        except exc.NoResultFound:
            raise q_exc.personalities_deltaNotFound(personalities_delta_id=id)
        except exc.MultipleResultsFound:
            LOG.error('Multiple personalities_deltas match for %s' % id)
            raise q_exc.personalities_deltaNotFound(personalities_delta_id=id)
        return personalities_delta
    ...
    def get_chains_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_chains_delta,
    ...
    def create_chains_delta(self, context, chains_delta):
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            chains_delta = ns_chains_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
                                         chain_id=n['id'],
                                         name=n['name'],
    ...
    def _get_chains_delta(self, context, id):
        try:
            chains_delta = self._get_by_id(context, ns_chains_delta, id)
        except exc.NoResultFound:
    ...
    def get_chain_appliances_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_chain_appliances_delta,
    ...
    def create_chain_appliances_delta(self, context, chain_appliances_delta):
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            chain_appliances_delta = ns_chain_appliances_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
                                         name=n['name'],
    ...
    def _get_chain_appliances_delta(self, context, id):
        try:
            chain_appliances_delta = self._get_by_id(context, ns_chain_appliances_delta, id)
        except exc.NoResultFound:
    ...
    ######################### Chains appliances Config Delta ###############################   
    def _make_chain_configuration_associates_delta_dict(self, chain_configuration_associates_delta, fields=None):
        res = {'id': chain_configuration_associates_delta['id'],
               'chain_appliance_map_config_id': chain_configuration_associates_delta['chain_appliance_map_config_id'],
               'name': chain_configuration_associates_delta['name'],
               'chain_map_id': chain_configuration_associates_delta['chain_map_id'],
               'config_handle_id': chain_configuration_associates_delta['config_handle_id'],
               'networkfunction_id': chain_configuration_associates_delta['networkfunction_id'],
               'tenant_id': chain_configuration_associates_delta['tenant_id'],
               'operation': chain_configuration_associates_delta['operation'],
               'user_id': chain_configuration_associates_delta['user_id'],
               'logged_at': chain_configuration_associates_delta['logged_at'],
               'version_id': chain_configuration_associates_delta['version_id']}
        return self._fields(res, fields)
    
    def get_chain_configuration_associates_delta(self, context, id, fields=None):
        chain_configuration_associates_delta = self._get_chain_configuration_associates_delta(context, id)
        return self._make_chain_configuration_associates_delta_dict(chain_configuration_associates_delta, fields)
        
    def get_chain_configuration_associates_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_chain_configuration_associates_delta,
                                    self._make_chain_configuration_associates_delta_dict,
                                    filters=filters, fields=fields)
    
    def create_chain_configuration_associates_delta(self, context, chain_configuration_associates_delta):
        n = chain_configuration_associates_delta['chain_configuration_associates_delta']
        LOG.error('Delta %s' % str(n))
        tenant_id = self._get_tenant_id_for_create(context, n)
        user_id = context.user_id
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            chain_configuration_associates_delta = ns_chain_configuration_associates_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
                                         chain_appliance_map_config_id=n['id'],
                                         name=n['name'],
                                         ...
                                         logged_at=datetime.datetime.now(),
                                         version_id=version_id)
            context.session.add(chain_configuration_associates_delta)
        payload = self._make_chain_configuration_associates_delta_dict(chain_configuration_associates_delta)
        method = n['operation']+"_chain_appliance_config"
        fanoutmsg = {}
        fanoutmsg.update({'method':method,'payload':payload})
        delta={}
        version = payload['version_id']
        delta[version] = fanoutmsg
        rpc.fanout_cast(context,'crd-consumer', rpc_proxy.make_msg('call_consumer',payload=delta))
        return self._make_chain_configuration_associates_delta_dict(chain_configuration_associates_delta)
    
    def _get_chain_configuration_associates_delta(self, context, id):
        try:
            chain_configuration_associates_delta = self._get_by_id(context, ns_chain_configuration_associates_delta, id)
        except exc.NoResultFound:
            raise q_exc.chain_configuration_associates_deltaNotFound(chain_configuration_associates_delta_id=id)
        except exc.MultipleResultsFound:
            LOG.error('Multiple chain_configuration_associates_deltas match for %s' % id)
            raise q_exc.chain_configuration_associates_deltaNotFound(chain_configuration_associates_delta_id=id)
        return chain_configuration_associates_delta
    ...
    def get_chain_bypass_rules_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_chain_bypass_rules_delta,
    ...
    def create_chain_bypass_rules_delta(self, context, chain_bypass_rules_delta):
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            chain_bypass_rules_delta = ns_chain_bypass_rules_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
    ...
    def _get_chain_bypass_rules_delta(self, context, id):
        try:
            chain_bypass_rules_delta = self._get_by_id(context, ns_chain_bypass_rules_delta, id)
        except exc.NoResultFound:
    ...
    def get_chainsets_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_chainsets_delta,
    ...
    def create_chainsets_delta(self, context, chainsets_delta):
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            chainsets_delta = ns_chainsets_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
                                         chainset_id= cbr['id'],
                                         name= cbr['name'],
                                         operation = cbr['operation'],
    ...
    def _get_chainsets_delta(self, context, id):
        try:
            chainsets_delta = self._get_by_id(context, ns_chainsets_delta, id)
        except exc.NoResultFound:
    ...
    def get_chainrules_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_chainrules_delta,
    ...
    def create_chainrules_delta(self, context, chainrules_delta):
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            chainrules_delta = ns_chainrules_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
                                         chain_rule_id = n['id'],
                                         name=n['name'],
    ...
    def _get_chainrules_delta(self, context, id):
        try:
            chainrules_delta = self._get_by_id(context, ns_chainrules_delta, id)
        except exc.NoResultFound:
    ...
    def get_chainmaps_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_chainmaps_delta,
    ...
    def create_chainmaps_delta(self, context, chainmaps_delta):
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            chainmaps_delta = ns_chainmaps_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
                                         chain_map_id = n['id'],
                                         name=n['name'],
                                         type='L2-Mode',
    ...
    def _get_chainmaps_delta(self, context, id):
        try:
            chainmaps_delta = self._get_by_id(context, ns_chainmaps_delta, id)
        except exc.NoResultFound:
    ...
    def get_appliance_instances_deltas(self, context, filters=None, fields=None):
        return self._get_collection(context, ns_appliance_instances_delta,
    ...
    def create_appliance_instances_delta(self, context, appliance_instances_delta):
        ...
        with context.session.begin(subtransactions=True):
            version_id = self.create_version(context, tenant_id)
            appliance_instances_delta = ns_appliance_instances_delta(tenant_id=tenant_id,
                                         id=uuidutils.generate_uuid(),
        ...
        payload['chain_appliance_id'] = payload['appliance_map_id']
        chain_appliances = self.get_chain_appliances_deltas(context, filters={'chain_appliance_map_id':[payload['chain_appliance_id']], 'operation': ['create']})
        LOG.debug('chain appliances = %s' % str(chain_appliances))
        if chain_appliances:
            payload['chain_id'] = chain_appliances[0]['chain_id']
    ...
    def _get_appliance_instances_delta(self, context, id):
        try:
            appliance_instances_delta = self._get_by_id(context, ns_appliance_instances_delta, id)
        except exc.NoResultFound:
